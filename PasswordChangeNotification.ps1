...
# do not uncomment without reason, only for testing!
# $users = get-aduser -filter {(Enabled -eq $true) -and (Name -like "*Chernyavsky")} -properties Name, PasswordNeverExpires, PasswordExpired, PasswordLastSet, EmailAddress | where { $_.passwordexpired -eq $false }
# Count Users
...
foreach ($user in $notifyUsers)
{
    ...
    # Subject Setting
    $subject="Your Domain password will expire $messageDays"
    # Email Body Set Here, Note You can use HTML, including Images.
    # examples here https://youtu.be/iwvQ5tPqgW0 
    $body ="
    <font face=""verdana"">
    <p>Dear $name,</p>
    <p> Your Domain password will expire $messageDays </p>
    <p> To change this password on a PC press <strong>CTRL ALT Delete</strong> and choose <strong>Change Password</strong>. </p>
    <p> This Domain password is only used in our internal network (to login on Windows PC, to connect with wired or wireless network and some other services), </p>
    <p> <strong>but not for Gmail or Slack</strong>. </p><br>
    <p> Thanks, </p>
    <p> <strong>#tech-support (Slack)</strong> | <a href=""mailto:tech-support@company.com"">tech-support@company.com</a> </p>
        </font>"
    ...
    try{
        ...
        if($interval)
        {
            ...
            if(($interval) -Contains($daysToExpire))
            {
                ...
                Send-Mailmessage -smtpServer $smtpServer -port $SMTPPort -UseSsl -Credential $Credentials -from $from -to $emailaddress -subject $subject -body $body -bodyasHTML -priority High -Encoding $textEncoding -ErrorAction Stop
...
if($logging)
{
	...
	$htmlbody = "C:\Scripts\body.html"
        $notifiedUsers | Sort-Object DaystoExpire | ConvertTo-Html -Property UserName,Name,EmailAddress,PasswordSet,DaysToExpire,ExpiresOn | Out-File -FilePath $htmlbody
#        $reportBody = "Password Expiry Report Attached"
        $reportBody = Get-Content -Path $htmlbody | Out-String
        try{
#            Send-Mailmessage -smtpServer $smtpServer -port $SMTPPort -UseSsl -Credential $Credentials -from $from -to $reportTo -subject $reportSubject -body $reportbody -bodyasHTML -priority High -Encoding $textEncoding -Attachments $logFile -ErrorAction Stop 
            Send-Mailmessage -smtpServer $smtpServer -port $SMTPPort -UseSsl -Credential $Credentials -from $from -to $reportTo -subject $reportSubject -body $reportbody -bodyasHTML -priority High -Encoding $textEncoding -ErrorAction Stop
        }
        catch{
            $errorMessage = $_.Exception.Message
...
Get-ChildItem -Path $logPath -Recurse -Force | Where-Object { !$_.PSIsContainer -and $_.CreationTime -lt $limit -and $_.name -like "*-PasswordLog.csv"} | Remove-Item -Force
